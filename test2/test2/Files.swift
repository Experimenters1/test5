...
fileprivate extension Files {
    ...
    var actionEdit: UIAction {
        return UIAction(title: "Edit", image: UIImage(systemName: "pencil")) { [weak self] action in
            ...
            let newFileNameTextField = UITextField()
            newFileNameTextField.placeholder = "Enter new name"
            newFileNameTextField.text = cell.newFileName.text
            
            alert.addTextField { textField in
                textField.placeholder = newFileNameTextField.placeholder
                textField.text = newFileNameTextField.text
            }
            ...
            let update = UIAlertAction(title: "Rename", style: .default) { [weak self] action in
                ...
                if self.searching {
                    ...
                    if indexPath.row < self.links.count {
                        ...
                        let oldFileName = (videoName as NSString).deletingPathExtension
                        newFileName = (updateName as NSString).deletingPathExtension
                        fileExtension = (videoName as NSString).pathExtension
                        let updatedVideoName = "\(newFileName).\(fileExtension)"
                        self.links[indexPath.row].name = updatedVideoName
                        ...
                        DispatchQueue.main.async {
                            ...
                            let destinationURL = documentsFolderURL.appendingPathComponent(updatedVideoName)
                            ...
                            do {
                                ...
                                print("Updated file name in Documents:", updatedVideoName)
                            } catch {
